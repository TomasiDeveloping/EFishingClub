...
import {THEME} from "../../constants/localStorage.const";

@Component({
  selector: 'app-dark-light-switch',
  templateUrl: './dark-light-switch.component.html',
  styleUrl: './dark-light-switch.component.scss'
})
export class DarkLightSwitchComponent implements AfterViewInit{

  @ViewChild('darkModeSwitch', { read: ElementRef }) element: ElementRef | undefined;

  private static readonly DARK_THEME_CLASS: 'dark-theme' = 'dark-theme';
  ...
  toggleControl: FormControl = new FormControl<boolean>(false);

  constructor(@Inject(DOCUMENT) private document: Document) {
    this.toggleControl.valueChanges.subscribe((darkMode): void => {
      darkMode ? this.setDarkTheme() : this.setLightTheme();
    });

    const userPreferredTheme: string | null = localStorage.getItem(THEME);
  ...
  private setDarkTheme(): void {
    ...
    this.document.documentElement.classList.add(DarkLightSwitchComponent.DARK_THEME_CLASS);
  }

  private setLightTheme(): void {
    localStorage.setItem(THEME, 'light');
    this.document.documentElement.classList.remove(DarkLightSwitchComponent.DARK_THEME_CLASS);
  }
